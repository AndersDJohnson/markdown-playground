...
1. The first input value is non-zero, and the condition in line 5 holds and ``x`` is incremented from ``1`` to ``2`` in line 6.
...
2. The first input value is zero, and the condition in line 5 does not hold.
